...
class KotlinCompileContext(val jpsContext: CompileContext) {
    ...
    fun reportUnsupportedTargets() {
        ...
        byKind.forEach { (kind, targets) ->
            ...
            val msg =
                if (kind == null) "$presentableChunksListString is not yet supported in IDEA internal build system. Please use Gradle to build them."
                else "$kind is not yet supported in IDEA internal build system. Please use Gradle to build $presentableChunksListString."
